...
export function Checkpoint({ question, id, passed, onPass }: {
  ...
  return <form className="workspace-checkpoint" onSubmit={(event) => {
    ...
    <p role="status" aria-live="polite">{attempt !== null ? `${correct ? "Correct." : "Not quite — try again."} ${question.explanation}` : passed ? "Understanding check passed on this browser. You can try it again." : "Choose an explanation, then check it before submitting the flag."}</p>
  </form>;
}
