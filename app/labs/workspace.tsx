...
type Lab = {
  id: string; title: string; campaign: string; mode: string; difficultyBand: string; minutes: number; prerequisites: string[]; subnet: string;
  ...
  return <main className="standalone-workspace">
    ...
    <div className="workspace-layout">
      ...
      <article className="workspace-lesson" key={selectedId}>
        <header><p className="eyebrow green">{selected.campaign} / {selected.mode} / {selected.difficultyBand} / {selected.minutes} MIN</p><h2>{selected.title}</h2><p>Recommended first: {selected.prerequisites.length ? selected.prerequisites.map((id) => labs.find((lab) => lab.id === id)?.title).join("; ") : "None — start here"}. Prerequisites are guidance, not locks.</p></header>
