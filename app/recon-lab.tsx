...
// Honors NEXT_PUBLIC_LAB_CONTROLLER_URL; otherwise keeps localhost cookies same-site.
const controllerUrl = localControllerUrl();
const toolboxUrl = process.env.NEXT_PUBLIC_LAB_TOOLBOX_URL ?? "http://127.0.0.1:7681";
...
export function ReconLab() {
  ...
  const filteredLabs = useMemo(() => {
    ...
    return labs.filter((lab) => {
      const moduleStory = storyFor(lab.id);
      const searchable = `${lab.title} ${moduleStory.mission} ${moduleStory.campaignName} ${lab.skills.join(" ")}`.toLowerCase();
    ...
    <main className="app-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => setView("home")} aria-label="Open RECON LAB home">
          <span className="brand-mark">R/</span><span><strong>RECON//LAB</strong><small>LEGACY SHARED RANGE</small></span>
        </button>
        <nav className="main-nav" aria-label="Primary navigation">
          <Link className="button compact primary" href="/">Current curriculum</Link>
      ...
      <section className="app-content">
        <aside className="legacy-notice" aria-label="Legacy curriculum"><strong>Legacy modules</strong> — These 18 shared-range modules keep their original progress and runtime. For the current learning path, <Link href="/">start with Lab 00</Link>. Nothing has been reset or migrated.</aside>
        ...
        {view === "home" && <section className="page home-page">
          <section className="setup-card"><div><p className="eyebrow green">CURRENT CURRICULUM</p><h2>Start the current beginner learning path</h2><p>Read the scenario, follow the evidence, reveal per-flag hints, and submit flags. Includes message recovery with Base64, hex, ROT13, MD5, and a toy cipher.</p></div><Link className="button primary" href="/">Open beginner lab workspace</Link></section>
        ...
        {view === "learn" && <section className="page learn-page">
          ...
          <div className="learn-layout"><aside className="module-browser"><label className="search"><span>⌕</span><input aria-label="Search learning modules" placeholder="Search modules or skills" value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="filter-row" role="group" aria-label="Filter by learning path">{tracks.map((item) => <button key={item} className={track === item ? "selected" : ""} onClick={() => setTrack(item)}>{item === "FOUNDATION" ? "BASE" : item}</button>)}</div><div className="module-list">{filteredLabs.map((lab) => { const done = (progress[lab.id]?.completedObjectives.length ?? 0) === lab.objectives.length; return <button key={lab.id} className={lab.id === selected.id ? "module-row active" : "module-row"} onClick={() => chooseLab(lab.id)}><span>{done ? "✓" : lab.number}</span><div><strong>{lab.title}</strong><small>{storyFor(lab.id).mission}</small></div><em className={difficultyClass(lab.difficulty)}>{lab.difficulty.slice(0, 3)}</em></button>; })}</div></aside>
            ...
            <aside className="lab-tools"><section className="tool-card"><div className="tool-heading"><div><p className="eyebrow">RANGE CONTROL</p><strong>{runtime === "running" ? "Your shared range is running" : "Ready when you are"}</strong></div><span className={connected ? "status-dot online" : "status-dot"} /></div><p>{connected ? "This shared range runs on your device in an isolated, no-egress Docker network." : "Connect the local controller to launch Docker targets and save progress."}</p><code>{selected.target}</code>{runtime === "running" && <a className="button compact secondary" href={toolboxUrl} target="_blank" rel="noreferrer">Open toolbox terminal</a>}</section><section className="hint-card"><div className="card-heading"><div><p className="eyebrow">NEED A NUDGE?</p><h2>Hints</h2></div><small>Score: {selectedProgress.score}</small></div>{selected.hints.map((hint, index) => { const visible = revealedHints.includes(hint.id) || selectedProgress.hints.includes(hint.id); return <div className="hint" key={hint.id}><span>0{index + 1}</span><div><strong>{visible ? `Hint ${index + 1}` : "Locked hint"}</strong>{visible && <p>{hint.body}</p>}</div><button disabled={visible} onClick={() => void unlockHint(hint)}>{visible ? "Open" : `-${hint.penalty} XP`}</button></div>; })}<button className="solution-button" onClick={() => void unlockSolution()} disabled={solutionVisible || selectedProgress.solutionUnlocked}>{solutionVisible || selectedProgress.solutionUnlocked ? "Walkthrough open" : "Open full walkthrough"}</button>{(solutionVisible || selectedProgress.solutionUnlocked) && <div className="solution-copy"><strong>Full walkthrough</strong><p>{selected.solution}</p><p>Reset the scenario and reproduce the finding without the walkthrough.</p></div>}</section><section className="notes-card"><div className="card-heading"><div><p className="eyebrow">YOUR NOTES</p><h2>Evidence log</h2></div><button className="text-button" onClick={() => void saveNotes()}>Save</button></div><textarea aria-label="Evidence notes" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={"What did you observe?\nWhat does it mean?\nWhat would you fix?"} /></section><section className="message-card"><span className={connected ? "status-dot online" : "status-dot"} /><p>{message}</p></section></aside>
          </div>
        </section>}

        {view === "library" && <section className="page library-page"><header className="page-heading"><p className="eyebrow green">DEFENSE LIBRARY</p><h1>Turn every finding into a fix</h1><p>Use the same lessons to understand how defenders discover, contain, and prevent common application risk.</p></header><div className="defense-grid">{labs.map((lab) => { const moduleStory = storyFor(lab.id); return <article key={lab.id} className="defense-entry"><div><span className="tag">{moduleStory.campaignName}</span><small>{lab.number}</small></div><h2>{lab.title}</h2><p className="defense-finding">{moduleStory.mission}</p><div><p><b>INVESTIGATE</b>{moduleStory.evidence}</p><p><b>PREVENT</b>{moduleStory.defense}</p></div><button className="text-button" onClick={() => chooseLab(lab.id)}>Open module →</button></article>; })}</div></section>}
